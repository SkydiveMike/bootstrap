...
main () {
    if [[ -d "$DOTINIT" ]] && [[ -r "$DOTINIT" ]]; then
        ...
        if [ $? = 1 ]; then # Proceed to Git Clone
            ...
            if ! GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i $private_key" git clone --recurse-submodules "$DOTINIT_REPO" "$DOTINIT" 2>&1; then
