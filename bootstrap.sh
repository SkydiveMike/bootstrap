...
bootstrap_main () {
    if [[ -d "$DOTINIT" ]] && [[ -r "$DOTINIT" ]]; then
        ...
        if ! GIT_SSH_COMMAND="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=quiet -i $private_key" sh -c "$(curl -fsLS git.io/chezmoi)" -d -b "$HOME/.local/bin" -- init --source "$DOTINIT" "$DOTINIT_REPO" 2>&1; then
